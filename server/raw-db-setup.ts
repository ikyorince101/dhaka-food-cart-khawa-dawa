...
export async function setupDatabaseWithRawConnection(): Promise<boolean> {
  console.log('🔧 Setting up database with raw connection...');
  
  try {
    // Create a simple pool for raw connection
    const pool = new Pool({
    ...
    console.log('✅ Raw database setup finished successfully');
    return true;
    
  } catch (error) {
    console.error('❌ Raw database setup failed:', error);
    return false;
  }
} 